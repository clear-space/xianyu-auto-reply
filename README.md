...
系统内置完整的数据治理机制，长期运行不会无限占用磁盘：

- **数据保留策略**：所有日志类数据（Token续期/自动回复消息/登录/发布/风控等 15+ 张表）由统一保留引擎自动清理，
  默认保留 **30 天**，可在「系统设置 → 数据保留策略」按表调整并手动触发立即清理
- **图片清理**：上传目录中的孤儿/超期图片每 20 分钟自动清理（卡券/素材/关键词/回复/人脸截图/公开接口媒体全覆盖）
- **临时文件清理**：进程异常退出遗留的滑块会话、发布图片、恢复上传残留每小时自动清扫
- **数据库备份**：每小时全库自动备份（不含系统账号表），超期备份自动清理（默认 10 天）
- **运行日志**：单文件 100MB 自动轮转，保留天数可配（默认 7 天）
- **系统信息看板**：「日志管理 → 系统信息」提供 CPU/内存/磁盘/网络实时状态与趋势、
  存储分布、数据库 TOP 表、数据保留策略生效状态、服务健康与阈值告警（磁盘默认 85% 告警）
- 客户服务器部署的存储规划与运维指引见 `personal_dev/客户部署说明-存储.md`
