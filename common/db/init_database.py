...
class DatabaseInitializer:
    ...
    DEFAULT_SCHEDULED_TASKS = (
        ...
        (
            ...
            600,
            True,
            "定期扫描按前缀编号关联卡券（仅处理开启自动关联开关的账号所属用户）",
        ),
        (
            "image_cleanup",
            "图片清理",
    ...
    TABLES_DDL = {
        ...
        "xy_accounts": """
            CREATE TABLE IF NOT EXISTS xy_accounts (
                ...
                auto_confirm TINYINT DEFAULT 0 COMMENT '自动确认发货',
                show_browser TINYINT DEFAULT 0 COMMENT '显示浏览器',
                ...
                scheduled_redelivery TINYINT NOT NULL DEFAULT 0 COMMENT '定时补发货开关',
                scheduled_rate TINYINT NOT NULL DEFAULT 0 COMMENT '定时补评价开关',
                auto_polish TINYINT NOT NULL DEFAULT 0 COMMENT '商品自动擦亮开关',
                confirm_before_send TINYINT NOT NULL DEFAULT 0 COMMENT '发货成功再发卡券开关',
                only_send_card TINYINT NOT NULL DEFAULT 0 COMMENT '只发卡券不确认发货开关',
                created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
        ...
        "xy_catalog_items": """
            CREATE TABLE IF NOT EXISTS xy_catalog_items (
                ...
                is_polished TINYINT DEFAULT 0 COMMENT '是否擦亮',
        ...
        "xy_orders": """
            CREATE TABLE IF NOT EXISTS xy_orders (
                ...
                is_bargain TINYINT DEFAULT 0 COMMENT '是否小刀',
                ...
                card_only_delivered TINYINT NOT NULL DEFAULT 0 COMMENT '仅发卡券流程是否已处理',
        ...
        "xy_cards": """
            CREATE TABLE IF NOT EXISTS xy_cards (
                ...
                use_no_logistics_form TINYINT NOT NULL DEFAULT 0 COMMENT '是否通过无需邮寄表单发货',
                delivery_count INT DEFAULT 0 COMMENT '发货次数',
                price VARCHAR(32) COMMENT '对接价格',
                is_dockable TINYINT DEFAULT 0 COMMENT '是否可对接',
                fee_payer VARCHAR(32) COMMENT '手续费支付方式：distributor-分销主支付，dealer-分销商支付',
                is_multi_spec TINYINT DEFAULT 0 COMMENT '是否多规格',
        ...
        "xy_default_replies": """
            CREATE TABLE IF NOT EXISTS xy_default_replies (
                ...
                reply_type VARCHAR(32) DEFAULT 'text' COMMENT '回复类型：text-文本(可附带图片)，api-接口，external_contact-站外联系方式',
                reply_content TEXT COMMENT '回复内容',
                ...
                reply_once TINYINT DEFAULT 0 COMMENT '只回复一次',
                created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
                INDEX idx_account_id (account_id),
                INDEX idx_item_id (item_id),
        ...
        "xy_feedback_messages": """
            CREATE TABLE IF NOT EXISTS xy_feedback_messages (
                ...
                content TEXT NOT NULL COMMENT '消息内容',
                is_admin TINYINT DEFAULT 0 COMMENT '是否为管理员消息',
                created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
        ...
        "xy_goofish_crawl_jobs": """
            CREATE TABLE IF NOT EXISTS xy_goofish_crawl_jobs (
                ...
                fetch_detail TINYINT DEFAULT 1 COMMENT '是否抓取详情',
        ...
        "xy_auto_rate_configs": """
            CREATE TABLE IF NOT EXISTS xy_auto_rate_configs (
                id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '主键ID',
                account_id VARCHAR(80) NOT NULL COMMENT '账号ID',
                enabled TINYINT DEFAULT 0 COMMENT '是否启用自动评价',
        ...
        "xy_dock_records": """
            CREATE TABLE IF NOT EXISTS xy_dock_records (
                ...
                status TINYINT DEFAULT 1 COMMENT '对接状态：1启用 0停用',
                disable_reason VARCHAR(255) DEFAULT NULL COMMENT '禁用原因',
                owner_disabled TINYINT NOT NULL DEFAULT 0 COMMENT '是否被上级禁用锁定：1是 0否',
                ...
                allow_sub_dock TINYINT DEFAULT 0 COMMENT '是否允许下级对接',
        ...
        "xy_product_materials": """
            CREATE TABLE IF NOT EXISTS xy_product_materials (
                ...
                support_pickup TINYINT NOT NULL DEFAULT 0 COMMENT '是否支持自提',
        ...
        "xy_auto_reply_message_logs": """
            CREATE TABLE IF NOT EXISTS xy_auto_reply_message_logs (
                ...
                default_reply_once TINYINT NOT NULL DEFAULT 0 COMMENT '默认回复是否仅回复一次',
        ...
        "xy_listing_monitor_tasks": """
            CREATE TABLE IF NOT EXISTS xy_listing_monitor_tasks (
                ...
                direct_order TINYINT NOT NULL DEFAULT 0 COMMENT '采集后是否直接下单（开启则新采集商品立即用下单账号下单后再入库）',
                is_enabled TINYINT NOT NULL DEFAULT 1 COMMENT '是否启用监控任务',
                is_deleted TINYINT NOT NULL DEFAULT 0 COMMENT '是否已删除（软删除）',
        ...
        "xy_listing_monitor_items": """
            CREATE TABLE IF NOT EXISTS xy_listing_monitor_items (
                ...
                is_dm_sent TINYINT NOT NULL DEFAULT 0 COMMENT '是否已发起私信（已处理，避免重复发送）',
                ...
                is_ordered TINYINT NOT NULL DEFAULT 0 COMMENT '是否已下单成功',
        ...
        "xy_delivery_block_rules": """
            CREATE TABLE IF NOT EXISTS xy_delivery_block_rules (
                ...
                enabled TINYINT NOT NULL DEFAULT 0 COMMENT '规则开关',
                priority INT NOT NULL DEFAULT 0 COMMENT '执行优先级（越小越先执行）',
                block_reason VARCHAR(500) DEFAULT NULL COMMENT '禁止发货原因（发给买家的消息）',
                auto_close_order TINYINT NOT NULL DEFAULT 0 COMMENT '命中后主动关闭订单',
                only_card_after_close TINYINT NOT NULL DEFAULT 0 COMMENT '关闭订单后继续发货（只发卡券）',
        ...
        "xy_shared_scan_workers": """
            CREATE TABLE IF NOT EXISTS xy_shared_scan_workers (
                ...
                cookie_saved TINYINT NOT NULL DEFAULT 0 COMMENT 'Cookie是否已保存到账号表',
                created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
        ...
        "xy_weight_algorithms": """
            CREATE TABLE IF NOT EXISTS xy_weight_algorithms (
                ...
                enabled TINYINT NOT NULL DEFAULT 1 COMMENT '是否启用',
                is_builtin TINYINT NOT NULL DEFAULT 0 COMMENT '是否系统内置（内置算法仅少量开关可调，其余参数只读，不可删除/停用，列表置顶）',
                created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
        ...
        "xy_ai_listing_configs": """
            CREATE TABLE IF NOT EXISTS xy_ai_listing_configs (
                ...
                image_enabled TINYINT NOT NULL DEFAULT 0 COMMENT '是否启用AI图片生成',
    ...
    COLUMN_MIGRATIONS = {
        ...
        "xy_offline_schedules": [
            ...
            ("enabled", "TINYINT NOT NULL DEFAULT 1 COMMENT '是否启用'", "delist_algorithm_id"),
        ...
        "xy_publish_schedules": [
            ...
            ("deduplicate_enabled", "TINYINT NOT NULL DEFAULT 0 COMMENT '去重开关（随机模式可用）'", "random_count"),
        ...
        "xy_weight_algorithms": [
            ("is_builtin", "TINYINT NOT NULL DEFAULT 0 COMMENT '是否系统内置（内置算法仅少量开关可调，其余参数只读，不可删除/停用，列表置顶）'", "enabled"),
            ("algorithm_type", "VARCHAR(32) NOT NULL DEFAULT 'heat_weight' COMMENT '算法类型：heat_weight-热度加权, delist_weight-下架加权'", "name"),
        ],
        # 商品指标快照表：CREATE TABLE IF NOT EXISTS 不会给旧版本建的表补新列，
        # 旧库缺列会导致采集 INSERT 整行失败（商品列表/下架权重全部无数据）——逐列存在性检查自愈
        "xy_item_stats_daily": [
            ("show_pv_1d", "INT DEFAULT NULL COMMENT '当日曝光次数（近24小时窗口）'", "stat_date"),
            ("show_uv_1d", "INT DEFAULT NULL COMMENT '当日曝光人数（近24小时窗口）'", "show_pv_1d"),
            ("ipv_1d", "INT DEFAULT NULL COMMENT '当日浏览次数（近24小时窗口）'", "show_uv_1d"),
            ("ipv_uv_1d", "INT DEFAULT NULL COMMENT '当日浏览人数（近24小时窗口）'", "ipv_1d"),
            ("chat_uv_1d", "INT DEFAULT NULL COMMENT '当日咨询人数（近24小时窗口）'", "ipv_uv_1d"),
            ("pay_ord_cnt_1d", "INT DEFAULT NULL COMMENT '当日支付笔数（近24小时窗口）'", "chat_uv_1d"),
            ("pay_byr_cnt_1d", "INT DEFAULT NULL COMMENT '当日支付买家数（近24小时窗口）'", "pay_ord_cnt_1d"),
            ("pay_amt_1d", "VARCHAR(32) DEFAULT NULL COMMENT '当日支付金额（近24小时窗口）'", "pay_byr_cnt_1d"),
            ("ipv_pay_ucvr_1d", "VARCHAR(16) DEFAULT NULL COMMENT '当日浏览支付转化率（近24小时窗口）'", "pay_amt_1d"),
            ("show_pv_7d", "INT DEFAULT NULL COMMENT '近7天曝光次数'", "ipv_pay_ucvr_1d"),
            ("show_uv_7d", "INT DEFAULT NULL COMMENT '近7天曝光人数'", "show_pv_7d"),
            ("ipv_7d", "INT DEFAULT NULL COMMENT '近7天浏览次数'", "show_uv_7d"),
            ("ipv_uv_7d", "INT DEFAULT NULL COMMENT '近7天浏览人数'", "ipv_7d"),
            ("chat_uv_7d", "INT DEFAULT NULL COMMENT '近7天咨询人数'", "ipv_uv_7d"),
            ("pay_ord_cnt_7d", "INT DEFAULT NULL COMMENT '近7天支付笔数'", "chat_uv_7d"),
            ("pay_byr_cnt_7d", "INT DEFAULT NULL COMMENT '近7天支付买家数'", "pay_ord_cnt_7d"),
            ("pay_amt_7d", "VARCHAR(32) DEFAULT NULL COMMENT '近7天支付金额'", "pay_byr_cnt_7d"),
            ("ipv_pay_ucvr_7d", "VARCHAR(16) DEFAULT NULL COMMENT '近7天浏览支付转化率'", "pay_amt_7d"),
            ("want_count", "INT DEFAULT NULL COMMENT '累计想要数（商品详情接口）'", "ipv_pay_ucvr_7d"),
            ("days_on_shelf", "INT DEFAULT NULL COMMENT '上架天数（采集时刻）'", "want_count"),
            ("post_dt", "VARCHAR(8) DEFAULT NULL COMMENT '上架日期 yyyyMMdd'", "days_on_shelf"),
            ("created_at", "DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间'", "post_dt"),
            ("updated_at", "DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间'", "created_at"),
        ],
        "xy_keyword_rules": [
        ...
        "xy_product_materials": [
            ...
            ("support_pickup", "TINYINT NOT NULL DEFAULT 0 COMMENT '是否支持自提'", "shipping_method"),
        ...
        "xy_listing_monitor_tasks": [
            ...
            ("direct_order", "TINYINT NOT NULL DEFAULT 0 COMMENT '采集后是否直接下单（开启则新采集商品立即用下单账号下单后再入库）'", "order_batch_size"),
        ],
        "xy_listing_monitor_logs": [
        ...
        "xy_listing_monitor_items": [
            ("is_dm_sent", "TINYINT NOT NULL DEFAULT 0 COMMENT '是否已私信'", "raw_json"),
            ("is_ordered", "TINYINT NOT NULL DEFAULT 0 COMMENT '是否已下单'", "is_dm_sent"),
        ...
        "xy_order_fallback_accounts": [
            ...
            ("is_deleted", "TINYINT NOT NULL DEFAULT 0 COMMENT '是否已删除（软删除）'", "account_ids"),
        ],
        "xy_collect_fallback_accounts": [
            ("category_id", "BIGINT DEFAULT NULL COMMENT '所属分类ID（NULL=未分类全局兜底）'", "owner_id"),
            ("is_deleted", "TINYINT NOT NULL DEFAULT 0 COMMENT '是否已删除（软删除）'", "account_ids"),
        ...
        "xy_accounts": [
            ...
            ("scheduled_redelivery", "TINYINT NOT NULL DEFAULT 0 COMMENT '定时补发货开关'", "disable_reason"),
            ("scheduled_rate", "TINYINT NOT NULL DEFAULT 0 COMMENT '定时补评价开关'", "scheduled_redelivery"),
            ("auto_polish", "TINYINT NOT NULL DEFAULT 0 COMMENT '商品自动擦亮开关'", "scheduled_rate"),
            ("confirm_before_send", "TINYINT NOT NULL DEFAULT 0 COMMENT '发货成功再发卡券开关'", "auto_polish"),
            ("send_before_confirm", "TINYINT NOT NULL DEFAULT 0 COMMENT '卡券发送成功再确认发货开关'", "confirm_before_send"),
            ("only_send_card", "TINYINT NOT NULL DEFAULT 0 COMMENT '只发卡券不确认发货开关'", "send_before_confirm"),
            ("auto_red_flower", "TINYINT NOT NULL DEFAULT 0 COMMENT '自动求小红花开关'", "send_before_confirm"),
            ("delivery_disabled", "TINYINT NOT NULL DEFAULT 0 COMMENT '禁止发货开关'", "auto_red_flower"),
            ("delivery_disabled_reason", "VARCHAR(500) DEFAULT NULL COMMENT '禁止发货原因'", "delivery_disabled"),
            ("auto_close_order", "TINYINT NOT NULL DEFAULT 0 COMMENT '主动关闭订单开关'", "delivery_disabled_reason"),
            ("delivery_only_card_after_close", "TINYINT NOT NULL DEFAULT 0 COMMENT '关闭订单后继续发货（只发卡券）'", "auto_close_order"),
            ...
            ("ai_reply_block_ordered_users", "TINYINT NOT NULL DEFAULT 0 COMMENT '已下单用户禁止AI回复'", "delivery_disabled_excluded_items"),
            ("refund_cancel_enabled", "TINYINT NOT NULL DEFAULT 0 COMMENT '退款订单注销开关'", "ai_reply_block_ordered_users"),
            ...
            ("agree_deliver_enabled", "TINYINT NOT NULL DEFAULT 0 COMMENT '同意后发货开关'", "refund_cancel_timeout"),
            ...
            ("auto_match_cards", "TINYINT NOT NULL DEFAULT 1 COMMENT '商品入库后自动按前缀编号关联卡券开关'", "auto_red_flower"),
        ],
        "xy_orders": [
            ("card_only_delivered", "TINYINT NOT NULL DEFAULT 0 COMMENT '仅发卡券流程是否已处理'", "delivery_fail_reason"),
            ("is_bargain", "TINYINT DEFAULT 0 COMMENT '是否小刀'", "account_name"),
            ...
            ("is_rated", "TINYINT DEFAULT 0 COMMENT '是否已评价'", "receiver_address"),
        ...
        "xy_cards": [
            ...
            ("use_no_logistics_form", "TINYINT NOT NULL DEFAULT 0 COMMENT '是否通过无需邮寄表单发货'", "delay_seconds"),
            ("price", "VARCHAR(32) COMMENT '对接价格'", "delivery_count"),
            ("is_dockable", "TINYINT DEFAULT 0 COMMENT '是否可对接'", "price"),
        ...
        "xy_dock_records": [
            ...
            ("owner_disabled", "TINYINT NOT NULL DEFAULT 0 COMMENT '是否被上级禁用锁定：1是 0否'", "disable_reason"),
            ...
            ("allow_sub_dock", "TINYINT DEFAULT 0 COMMENT '是否允许下级对接'", "source_user_id"),
        ...
        "xy_catalog_items": [
            ...
            ("is_polished", "TINYINT DEFAULT 0 COMMENT '是否擦亮'", "ai_prompt"),
            ("updated_at", "DATETIME COMMENT '更新时间'", "created_at"),
        ],
        "xy_announcements": [
            ("is_deleted", "TINYINT NOT NULL DEFAULT 0 COMMENT '是否已删除'", "content"),
        ],
        "xy_card_item_relations": [
    ...
    DEFAULT_DELIST_ALGORITHMS = (
        ...
        (
            "保守保值",
            "谨慎下架：老化计分缓慢，近7天成交/流量/咨询与累计想要保护最强，尽量只下长期无成交商品",
            {
                "base_score": 100,
                "w_age": 50,
                "w_no_sale": 70,
