  ...
  // ══════════════════════════════════════════════════════
  {
    ...
    children: [
      ...
      },
      {
        ...
        icon: Clock,
        title: '定时发布',
        description: '创建定时规则，到时间自动触发批量发布。支持指定发布和随机发布两种模式。',
        steps: [
          { title: '新建规则', content: '点击「新建规则」按钮，设置规则名称和重复模式（单次/每天/每周）。' },
          { title: '配置时间', content: '选择「指定时间点」在固定时刻触发，或「时间段随机」在时间范围内随机触发。' },
          {
            title: '发布模式 - 指定发布',
            content: '将所选素材全部发布到所有选中账号。适合需要精确控制发布内容的场景。',
          },
          {
            title: '发布模式 - 随机发布',
            content: '每次触发时从素材池中随机选取指定数量的素材发布。可开启「去重」确保同一账号不会重复发布相同素材。\n\n适用场景：\n• 大量素材库随机推广\n• 避免账号发布重复内容被平台风控\n• 模拟真人操作的随机性',
          },
          { title: '管理规则', content: '在规则列表中可启用/禁用、编辑、删除规则。点击「立即执行」可手动触发一次。' },
          { title: '查看执行历史', content: '切换到「执行历史」Tab 查看每条规则的执行记录和实时进度。' },
          { title: '注意事项', content: '• 随机模式下素材池至少需要包含随机数量以上的素材\n• 开启去重后，已发布过的素材不会再被选中\n• 单次模式执行完毕后会自动禁用\n• 指定发布模式下去重功能不生效' },
        ],
      },
    ],
  },
