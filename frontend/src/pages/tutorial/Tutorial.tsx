      ...
      },
      {
        ...
        children: [
          ...
          },
          {
            ...
            icon: PackageX,
            title: '下架权重算法（下架加权）',
            description: '定义定时下架的选品权重算法：按上架天数、订单与擦亮情况为在售商品打分，规则引用算法决定下架优先级。',
            steps: [
              {
                title: '查看算法列表',
              ...
              {
                title: '新建/编辑算法',
                content: '设置算法名称与说明，调整权重参数：基础分、上架老化加分（含计分封顶）、无订单加分（含计分封顶）、近期订单扣分、擦亮扣分、下架分数线、硬排开关，以及选取方式。'
              },
              {
                title: '选择选取方式',
              ...
              {
                title: '预览算法效果',
                content: '点击算法行的「效果」按钮，查看该算法对当前账号全部在售商品的下架权重排名：每行展示逐项分值构成（基础/老化/无单/有单/擦亮）与信号徽章（上架天数/无单天数/近期有单/已擦亮）。'
              },
              {
                title: '查看引用规则',
            ...
            important: '内置算法「下架均衡」仅硬排开关与选取方式可调整，其余参数只读，不可删除/停用；被规则引用的算法无法删除（可先停用，停用后引用它的规则自动回退默认参数）。',
            tips: [
              '权重公式：基础分 + min(上架天数,封顶)×老化加分 + min(无订单天数,封顶)×无单加分 − 近期订单扣分 − 擦亮扣分，下限 0 分（0=不参与下架）',
              '无订单天数 = 最近订单距今；从未有单的商品按上架天数计',
