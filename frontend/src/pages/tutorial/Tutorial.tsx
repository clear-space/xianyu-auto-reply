    ...
    children: [
      ...
      },
      {
        ...
        steps: [
          ...
          {
            title: '设置筛选参数',
            content: '上架超过 X 天、最近无订单 Y 天（0=不检查订单）、每账号下架上限 Z 个。参数下方会实时显示当前条件的完整说明。'
          },
          {
            title: '选择账号',
            content: '仅下架所选账号的商品。每个账号每次最多下架 Z 个，上架最久的优先。'
          },
        ],
        tips: [
  ...
  // ══════════════════════════════════════════════════════
  {
    ...
    children: [
      ...
      },
      {
        ...
        icon: Settings2,
        title: '优化算法',
        description: '（管理员功能）算法优化相关配置，目前包含上架权重算法。',
        children: [
          {
            id: 'admin-weight-algorithms',
            icon: Scale,
            title: '上架权重算法',
            description: '定义定时发布随机模式的选料权重算法：按使用历史与售卖情况为素材打分，规则引用算法决定选料优先级。',
            steps: [
              {
                title: '查看算法列表',
              ...
              {
                title: '新建/编辑算法',
                content: '设置算法名称与说明，调整权重参数：首次使用加成、近30天订单加成、已售出加成、下架/删除恢复速率、失败扣分、硬排已售出，以及选料方式。'
              },
              {
                title: '选择选料方式',
              ...
              {
                title: '预览算法效果',
                content: '点击算法行的「效果」按钮，查看该算法对当前账号全部素材的权重排名：每行展示逐项分值构成（基础/首次使用/订单/售出/下架/删除/失败）与信号徽章；本地在售的素材标记为「在售（去重过滤）」，执行时会先被硬过滤。'
              },
              {
                title: '查看引用规则',
            ...
            tips: [
              '权重公式：100 基础分 + 首次使用加成 + 近30天订单加成 + 已售出加成 − 下架/删除恢复惩罚 − 失败扣分，保底 1 分',
              '被下架/删除的素材会随时间逐步恢复权重（默认 50 天 / 100 天回满），素材池永不枯竭',
            ],
          },
        ],
      },
