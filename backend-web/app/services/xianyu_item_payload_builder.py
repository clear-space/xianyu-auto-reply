...
def build_category_info(item_data: dict[str, Any]) -> dict[str, Any]:
...
async def build_item_payload(
    ...
    *,
    ...
    # 编辑场景（有平台快照）不动：快照描述可能已含合并标题，且编辑表单以平台原文为准。
    if snapshot is None:
        description = f"{title}\n\n{description}"
    ...
    category_info = build_category_info(item_data)
...
__all__ = ["DirectPublishError", "build_category_info", "build_item_payload"]
